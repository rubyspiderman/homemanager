class ReportController < ApplicationController
    ...
    def to_pdf(binder,user)
      ...
      if(binder.contractors.length > 0)      
        ...
        binder.contractors.each do |contractor|
          ...
          caddr = ''
          if contractor.address.address1.length > 0
            caddr += "#{contractor.address.address1}\n"
          end 
          if contractor.address.address2.length > 0
            caddr += "#{contractor.address.address2}\n"
          end
          if contractor.address.city.length > 0
            caddr += "#{contractor.address.city}, "
          end 
          if contractor.address.state.length > 0
            caddr += "#{contractor.address.state}, "
          end
          if contractor.address.zip.length > 0
      ...
      if(binder.paints.length > 0)                  
        ...
        binder.paints.each do |paint|
          ...
          pmanu = paint.paint_manufacturer unless paint.paint_manufacturer.nil?
  ...
  def index
    ...
    output    = HomeBinderReport.new.to_pdf(working_binder, working_user)
    
    date      = Date.today
    time      = Time.now
    pdfname   = "HomeBinder_Report_#{working_binder.name}_#{time}.pdf"


    respond_to do |format|
      format.pdf do
        send_data output, filename: pdfname, 
